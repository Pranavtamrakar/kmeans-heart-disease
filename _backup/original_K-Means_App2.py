...
# Load the exact column schema the model was trained on (source of truth)
...
# Any column the user doesn't see (e.g. id, restecg_0, slope_0, thal_0) defaults to 0.
# The "Heart Disease Stage" selectbox is mapped back to the 'num' column the model expects.
model_row = {col: 0 for col in FEATURE_COLUMNS}

# Numeric fields the user filled in
for feature in numeric_features:
    if feature in model_row:
        model_row[feature] = user_inputs[feature]

# Map the UI's "Heart Disease Stage" choice -> the model's 'num' column
hds_choice = next(
...
# Copy across the normal one-hot categoricals (skip the renamed Heart Disease Stage one)
